...
const AdjacentPostCard = ({ post, position }) => (
  <>
    ...
    <div className="absolute rounded-lg bg-center bg-gradient-to-b opacity-60 from-gray-400 via-gray-700 to-black w-full h-full" />
    <div className="flex flex-col rounded-lg p-4 items-center justify-center absolute w-full h-full">
      <p className="text-white text-shadow font-semibold text-xs">{moment(post.createdAt).format('MMM DD, YYYY')}</p>
      <p className="text-white text-shadow font-semibold text-lg lg:text-2xl text-center line-clamp-2 overflow-hidden text-ellipsis">{post.title}</p>
    </div>
